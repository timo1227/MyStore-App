...
if ($user_id > 0) {
    $db = getDB();
    $stmt = $db->prepare("SELECT name, c.id as line_id, item_id, quantity, cost, (cost*quantity) as subtotal FROM RM_Cart_Alt c JOIN RM_Items i on c.item_id = i.id WHERE c.user_id = :uid");
    try {
        ...
        if ($total_cost) {
            ...
            if ($next_order_id > 0) {
                ...
                set stock = stock - (select IFNULL(quantity, 0) FROM RM_Cart_Alt WHERE item_id = RM_Items.id and user_id = :uid) 
                WHERE id in (SELECT item_id from RM_Cart_Alt where user_id = :uid)");
                try {
                    $stmt->execute([":uid" => $user_id]);
                } catch (PDOException $e) {
            ...
            if ($next_order_id > 0) {
                ...
                SELECT item_id, user_id, RM_Cart.quantity, cost, :order_id FROM RM_Cart_Alt JOIN RM_Items on RM_Cart.item_id = RM_Items.id
                 WHERE user_id = :uid");
                try {
            ...
            if ($next_order_id > 0) {
                $stmt =  $db->prepare("DELETE from RM_Cart_Alt where user_id = :uid");
                try {
                    $stmt->execute([":uid" => $user_id]);
                } catch (PDOException $e) {
